...
跟随与强制弹出分开：文档工具窗**已经可见**且分栏开启时，切换源文件/光标才更新面板内容；工具窗隐藏时打开或切换代码**不会** `show()` 弹窗。`forceFocus`（`Ctrl+Alt+D`、Inlay、Initialize、主页点文档等）才 `activate()` 打开面板。无绑定且面板已开时显示新建入口。代码块选区进行中、以及从文档跳到源码的短窗口内不同步，以免抢走编辑器焦点。
