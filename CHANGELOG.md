...
- 保存源码后的哈希提醒改为右下角通知，避免在 VFS 写锁上弹出模态框把 IDE 冻死
- 安装说明改为优先从 JetBrains Marketplace 安装

## 0.1.19

- 文档面板从隐藏再次打开时，按当前源文件同步文档，不再沿用隐藏期间的旧内容

## 0.1.18

- 文档跟随与强制弹出解耦：面板已打开时切换源文件才更新文档；面板隐藏时打开代码不再自动弹出

## 0.1.17

- 插件简介补充中文说明（仍以英文开头，满足市场校验）
- 命令增加中文 `description`
- 使用说明改为完整中文手册，不再把日常用法推到 VS Code 仓库
